...
def log_validation(
    ...
    logger.info(
        ...
        f"\n into {os.path.join(args.output_dir, 'images')}"
    )
    ...
    pipeline = SwiftBrushPipeline.from_pretrained(
    ...
    for i, image in enumerate(images):
        image.save(
            os.path.join(
                args.output_dir,
                "images",
                f"{args.validation_prompt}_step-{global_step}_{i}.png",
            )
        )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                timesteps = torch.ones(
                    (bsz,), dtype=torch.int64, device=model_input.device
                )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        accelerator.save_state(save_path, no_safe_serialization=True)
