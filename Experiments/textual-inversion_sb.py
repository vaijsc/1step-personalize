...
PRETRAINED_SB_PATH = "/root/research/1-step-personalize/SB/ckpts/sbv2_fid81"
# ------------------------------------------------------------------------------
...
@torch.no_grad()
def inference(
    vae, unet, noise_scheduler, encoded_embeds, args, generator, device, weight_dtype
):
...
def log_validation(
    ...
    validation_prompt = args.validation_prompt.format(args.placeholder_token)
    logger.info(
    ...
    for i, image in enumerate(images):
        save_image = transforms.functional.to_pil_image(torch.squeeze_copy(image))
        save_image.save(
            os.path.join(
                args.output_dir,
                "images",
                f"{validation_prompt}_step-{global_step}_{i}.png",
            )
        )
...
def main():
    ...
    alphas_cumprod = noise_scheduler.alphas_cumprod.to(accelerator.device, weight_dtype)
    alpha_t = (alphas_cumprod[T] ** 0.5).view(-1, 1, 1, 1)
    sigma_t = ((1 - alphas_cumprod[T]) ** 0.5).view(-1, 1, 1, 1)
    del alphas_cumprod

    last_timestep = torch.ones((1,), dtype=torch.int64, device=accelerator.device) * (
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(text_encoder):
                ...
                timesteps = torch.ones((bsz,), dtype=torch.int64, device=latents.device)
                timesteps = timesteps * (noise_scheduler.config.num_train_timesteps - 1)

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                # noisy_latents = alpha_t * latents + sigma_t * noise

                ### TESTING DRAWING FROM RANDOM NOISE WITHOUT PERTUBATION
                noisy_latents = noise
                ###
                ...
                model_pred = unet(
                    noisy_latents, last_timestep, encoder_hidden_states
                ).sample
