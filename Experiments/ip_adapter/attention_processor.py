...
class SequenceIPAttnProcessor2_0(IPAdapterAttnProcessor2_0):
    def __init__(
        self,
        ipa_module: IPAdapterAttnProcessor2_0,
        device="cpu",
        dtype=torch.float32,
    ):
        super().__init__(
            ipa_module.hidden_size,
            ipa_module.cross_attention_dim,
            ipa_module.num_tokens,
            ipa_module.scale,
        )
        self.load_state_dict(ipa_module.state_dict())
    ...
    #     # Update query with text attention results
    #     query = attn.to_q(hidden_states)
    #     query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

    #     # Then attend to IP image
    #     ip_key = self.to_k_ip(ip_hidden_states)
    #     ip_value = self.to_v_ip(ip_hidden_states)
    ...
    #     # Scale only the IP attention results
    #     hidden_states = hidden_states # * self.scale (Further testing in the future)
    ...
    def __call__(
        ...
        # Reset hidden_states to use only IP features
...
def replace_ipa(unet, device, dtype):
    attn_processors = unet.attn_processors
    for name, processor in unet.attn_processors.items():
        if isinstance(processor, IPAdapterAttnProcessor2_0):
            attn_processors[name] = SequenceIPAttnProcessor2_0(processor, device, dtype)
