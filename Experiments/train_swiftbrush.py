...
DATASET_PATH = "/root/research/1-step-personalize/Data/COCA_dataset_large"
DATA_JSON_FILE = "unique_info.json"
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.data = json.load(
            open(os.path.join(DATASET_PATH, json_file))
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="/root/research/1-step-personalize/Data/dreambench/can/00.jpg",
        help="The path to a text file containing all validation prompts",
    )
    parser.add_argument(
        "--validation_prompts",
        type=str,
...
def inference(
    ...
    pred_original_sample = predict_original(
        ip_adapter,
        noise_scheduler,
        input_noise,
        prompt_embed,
...
def predict_original(
    ip_adapter, noise_scheduler, input_noise, prompt_embeds, image_embeds
):
    ...
    model_pred = ip_adapter(input_noise, max_timesteps, prompt_embeds, image_embeds)
    ...
    def __getitem__(self, index):
        data = {
            "prompt_embeds": torch.from_numpy(
                np.load(self.train_data_paths[index], allow_pickle=True)
            )
        }
        return data
...
def main(args):
    ...
    # load pretrained dmd2
    unet.load_state_dict(
        torch.load(
            hf_hub_download(
                "tianweiy/DMD2",
                "model/sdv1.5/laion6.25_sd_baseline_8node_guidance1.75_lr5e-7_seed10_dfake10_diffusion1000_gan1e-3_resume_fid8.35_checkpoint_model_041000/pytorch_model.bin",
            ),
            map_location="cuda",
        )
    )
    ...
    teacher_image_proj_model = ImageProjModel(
        cross_attention_dim=teacher_lora.config.cross_attention_dim,
        clip_embeddings_dim=image_encoder.config.projection_dim,
        clip_extra_context_tokens=4,
    )
    teacher_image_proj_model.requires_grad_(False)

    # init student adapter modules
    # student ipa
    attn_procs = {}
    ...
    ip_adapter = IPAdapter(
        ...
        image_proj_model,
        adapter_modules,
        "/root/.cache/huggingface/hub/models--h94--IP-Adapter/snapshots/018e402774aeeddd60609b4ecdb7e298259dc729/models/ip-adapter_sd15.bin",
    )

    # teacher ipa
    teacher_attn_procs = {}
    teacher_unet_sd = teacher_lora.state_dict()
    for name in teacher_lora.attn_processors.keys():
        cross_attention_dim = (
            None
            if name.endswith("attn1.processor")
            else teacher_lora.config.cross_attention_dim
        )
        if name.startswith("mid_block"):
            hidden_size = teacher_lora.config.block_out_channels[-1]
        elif name.startswith("up_blocks"):
            block_id = int(name[len("up_blocks.")])
            hidden_size = list(reversed(teacher_lora.config.block_out_channels))[
                block_id
            ]
        elif name.startswith("down_blocks"):
            block_id = int(name[len("down_blocks.")])
            hidden_size = teacher_lora.config.block_out_channels[block_id]
        if cross_attention_dim is None:
            teacher_attn_procs[name] = AttnProcessor()
        else:
            layer_name = name.split(".processor")[0]
            weights = {
                "to_k_ip.weight": teacher_unet_sd[layer_name + ".to_k.weight"],
                "to_v_ip.weight": teacher_unet_sd[layer_name + ".to_v.weight"],
            }
            teacher_attn_procs[name] = IPAttnProcessor(
                hidden_size=hidden_size, cross_attention_dim=cross_attention_dim
            )
            teacher_attn_procs[name].load_state_dict(weights)
    teacher_lora.set_attn_processor(teacher_attn_procs)
    teacher_adapter_modules = torch.nn.ModuleList(teacher_lora.attn_processors.values())
    teacher_adapter_modules.requires_grad_(False)
    ...
    teacher_ip_adapter = IPAdapter(
        teacher_lora,
        teacher_image_proj_model,
        teacher_adapter_modules,
        "/root/.cache/huggingface/hub/models--h94--IP-Adapter/snapshots/018e402774aeeddd60609b4ecdb7e298259dc729/models/ip-adapter_sd15.bin",
    )
            ...
            if accelerator.is_main_process:
                ...
                for i, model in enumerate(models):
                    if isinstance(model, type(accelerator.unwrap_model(unet))):
                        ...
                        model, type(accelerator.unwrap_model(teacher_lora))
                    ):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                if isinstance(model, type(accelerator.unwrap_model(unet))):
                    ...
                    load_model = PeftModel.from_pretrained(
                        base_model, input_dir, subfolder="teacher_lora"
                    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("swiftbrush_IPA", config=tracker_config)
    ...
    ########## GET VALIDATION IMAGE EMBEDS ##########
    val_image = Image.open(args.validation_image_path).convert("RGB")
    clip_image_processor = CLIPImageProcessor()

    val_clip_image = clip_image_processor(
        images=val_image, return_tensors="pt"
    ).pixel_values
    val_clip_image = val_clip_image.to(accelerator.device, dtype=weight_dtype)

    with torch.no_grad():
        val_image_embeds = image_encoder(val_clip_image).image_embeds
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(ip_adapter, teacher_ip_adapter):
                bsz = batch["text_input_ids"].shape[0]
                ...
                with torch.no_grad():
                    accelerator.unwrap_model(
                        teacher_ip_adapter.module.unet
                    ).disable_adapters()
                    teacher_pred_cond = teacher_ip_adapter(
                        noisy_samples, timesteps, prompt_embeds, image_embeds
                    )
                    teacher_pred_uncond = teacher_ip_adapter(
                        noisy_samples, timesteps, prompt_null_embeds, image_embeds
                    )

                    accelerator.unwrap_model(
                        teacher_ip_adapter.module.unet
                    ).enable_adapters()
                    lora_pred_cond = teacher_ip_adapter(
                        noisy_samples, timesteps, prompt_embeds, image_embeds
                    )
                    lora_pred_uncond = teacher_ip_adapter(
                        noisy_samples, timesteps, prompt_null_embeds, image_embeds
                    )
                    ...
                    lora_pred = lora_pred_uncond + args.guidance_scale * (
                ...
                # (this is the forward diffusion process)
                noisy_samples = noise_scheduler.add_noise(
                    pred_original_samples.detach(), noise, timesteps
                )
                ...
                lora_pred = teacher_ip_adapter(
                    noisy_samples, timesteps, encoder_hidden_states, image_embeds
                )
                        ...
                        if (
                            ...
                            with torch.cuda.amp.autocast():
                                ...
                                for prompt, validation_dict in zip(
                                    args.validation_prompts, validation_dicts
                                ):
                                    images[prompt] = [
                                        inference(
                                            vae,
