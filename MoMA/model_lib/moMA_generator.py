...
from diffusers import StableDiffusionPipeline, DDIMScheduler, AutoencoderKL, UNet2DConditionModel, LCMScheduler
import tqdm
...
class DMD2_MoMA_generator:
    def __init__(self, device,args):
        ...
        print('Loading StableDiffusion: DMD2...')
        ...
        self.feat_extract_noise_scheduler = DDIMScheduler(num_train_timesteps=1000,beta_start=0.00085,beta_end=0.012,beta_schedule="scaled_linear",clip_sample=False,set_alpha_to_one=False,steps_offset=1,)
        self.feat_extract_unet = UNet2DConditionModel.from_pretrained(
            "runwayml/stable-diffusion-v1-5",
            subfolder="unet",
        ).to(
            "cuda",
    ...
    def get_image_selfAttn_feature(
        ...
        noise = torch.randn_like(latents)
        timesteps = torch.tensor([0],device=latents.device).long() # fixed to 0
        noisy_latents = self.feat_extract_noise_scheduler.add_noise(latents, noise, timesteps)

        _ = self.feat_extract_unet(noisy_latents,timestep=timesteps,encoder_hidden_states=text_embeddings)["sample"]
        # Copy self attention features from feat_extract_unet to self.unet
        for name, attn_processor in self.feat_extract_unet.attn_processors.items():
            if isinstance(attn_processor, IPAttnProcessor_Self):
                # Get corresponding processor from main unet
                main_processor = self.unet.attn_processors[name]
                if isinstance(main_processor, IPAttnProcessor_Self):
                    # Copy the extracted features
                    main_processor.store_ks, main_processor.store_vs = attn_processor.store_ks.clone(), attn_processor.store_vs.clone()
        # features are stored in attn_processors
