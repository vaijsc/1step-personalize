...
from .attention_processor import (
    MaskedIPAttnProcessor2_0 as IPAttnProcessor,
...
class MSAdapter(torch.nn.Module):
    def __init__(
        ...
        # dummy image tokens
        self.dummy_image_tokens = nn.Parameter(
            torch.randn(1, self.num_dummy_tokens, self.cross_attention_dim)
        )
    ...
    def load_from_checkpoint(self, ckpt_path: str):
        ...
        self.load_state_dict(
            {"dummy_image_tokens": state_dict["dummy_image_tokens"]}, strict=False
        )
            ...
            if isinstance(self.controlnet, MultiControlNetModel):
                for controlnet in self.controlnet.nets:
                    controlnet.set_attn_processor(
                        CNAttnProcessor(
                            text_tokens=self.text_tokens, num_tokens=self.num_tokens
                        )
                    )
            else:
    ...
    def get_image_embeds(
        ...
        if image_proj_type == "resampler":
            image_embeds = image_encoder(
                ...
                -2
            ]  # (bsz*rn, num_tokens, embedding_dim)
        else:
            image_embeds = image_encoder(
                processed_images.to(self.device, dtype=weight_dtype)
            ).image_embeds  # (bsz*rn, embedding_dim)
        ...
        **kwargs,
        ...
        if boxes is not None:
            ...
            if phrases is not None:
                drop_grounding_tokens = (
                    drop_grounding_tokens
                    if drop_grounding_tokens is not None
                    else [0] * bsz
                )
                ...
                for phrase in phrases:
                    phrase_input_id = pipe.tokenizer(
                        phrase,
                ...
                phrase_embeds = pipe.text_encoder(
                    phrase_input_ids.to(self.device)
                ).pooler_output
                grounding_kwargs = {
                    "boxes": batch_boxes,
        ...
        if phrase_idxes is not None:
            ...
            phrase_idxes = torch.repeat_interleave(
                phrase_idxes, repeats=num_samples, dim=0
            )
        ...
        with torch.inference_mode():
            ...
            image_prompt_embeds = self.image_proj_model(
                image_embeds, grounding_kwargs=grounding_kwargs
            )
            image_prompt_embeds = image_prompt_embeds.view(
                bsz, -1, image_prompt_embeds.shape[-2], image_prompt_embeds.shape[-1]
            )  # (bsz, rn, num_tokens, cross_attention_dim)
            image_prompt_embeds = image_prompt_embeds.view(
                bsz,
                image_prompt_embeds.shape[-3] * image_prompt_embeds.shape[-2],
                image_prompt_embeds.shape[-1],
            )  # (bsz, total_num_tokens*rn, cross_attention_dim)
            image_prompt_embeds = torch.cat(
                [self.dummy_image_tokens, image_prompt_embeds], dim=1
            )
