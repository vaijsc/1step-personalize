...
class MaskedIPAttnProcessor2_0(nn.Module):
    ...
    def __init__(
        ...
        num_dummy_tokens=4,
        mask_threshold=0.5,
        use_psuedo_attention_mask=False,
        subject_scales=None,
        start_step=5,
    ):
        super().__init__()
    ...
    def prepare_attention_mask_qk(
        ...
        dummy_attention_mask = torch.ones(
            (batch_size, sequence_length_q), dtype=dtype, device=device
        )
        for box_idx in box_idxes.unbind(dim=1):
            x_start_patch_idx, y_start_patch_idx, x_end_patch_idx, y_end_patch_idx = (
                box_idx.unbind(dim=1)
            )
            x_indices = (
                torch.arange(num_patches_per_row)
                .unsqueeze(0)
                .expand(batch_size, -1)
                .to(device)
            )
            ...
            x_mask = (
                (x_indices >= x_start_patch_idx.unsqueeze(1))
                & (x_indices < x_end_patch_idx.unsqueeze(1))
            ).to(dtype)
            y_mask = (
                (y_indices >= y_start_patch_idx.unsqueeze(1))
                & (y_indices < y_end_patch_idx.unsqueeze(1))
            ).to(dtype)
            box_mask = torch.bmm(y_mask.unsqueeze(2), x_mask.unsqueeze(1)).reshape(
                batch_size, -1
            )
        ...
        post_dummy_attention_mask = post_dummy_attention_mask.repeat_interleave(
            head_size, dim=0
        )

        attention_mask_qk_image = torch.stack(box_masks, dim=-1)
        attention_mask_qk_image = attention_mask_qk_image.repeat_interleave(
            self.num_tokens, dim=-1
        )
        attention_mask_qk_image = (
            1 - attention_mask_qk_image.to(dtype)
        ) * -10000.0  # mask to bias
        # use dummy image tokens to process the background
        dummy_attention_mask = dummy_attention_mask.unsqueeze(-1).repeat_interleave(
            self.num_dummy_tokens, dim=-1
        )
        dummy_attention_mask = (1 - dummy_attention_mask) * -10000.0
        attention_mask_qk_image = torch.cat(
            [dummy_attention_mask, attention_mask_qk_image], dim=-1
        )
        ...
        if use_masked_text_attention:
            attention_mask_qk_text = torch.ones(
            ...
            for i in range(batch_size):
                for j in range(len(box_masks)):
                    start_idx, end_idx = int(phrase_idxes[i, j, 0].item()), int(
                        phrase_idxes[i, j, 1].item()
                    )
                    if start_idx == 0 and end_idx == 0:
                        continue
                    attention_mask_qk_text[i, :, start_idx:end_idx] = box_masks[j][
                        i, ...
                    ].unsqueeze(-1)
            attention_mask_qk_text = (1 - attention_mask_qk_text) * -10000.0
            if attention_mask_qk_text.shape[0] < batch_size * head_size:
                attention_mask_qk_text = attention_mask_qk_text.repeat_interleave(
                    head_size, dim=0
                )
        else:
        ...
        return (
            attention_mask_qk_image,
            attention_mask_qk_text,
            post_dummy_attention_mask,
        )
    ...
    def get_text_attention_maps(self, attention_probs, boxes, phrase_idxes, head_size):
        ...
        attention_probs = attention_probs.view(
            bsz, head_size, num_tokens_q, num_tokens_k
        )
        ...
        for i in range(bsz):
            ...
            for j in range(num_ref):
                ...
                if start_idx == 0 and end_idx == 0:
                    ...
                    attention_map = attention_probs[
                        i, :, :, start_idx:end_idx
                    ]  # [num_heads, num_tokens_q, num_tokens_phrase]
                    attention_map = torch.mean(
                        torch.mean(attention_map, dim=-1), dim=0
                    )  # [num_tokens_q]
        ...
        self.attention_maps.append(
            torch.stack(batch_attention_maps).reshape(bsz, num_ref, h, w)
        )

    def get_psuedo_attention_mask(self, head_size):
        # text_attention_maps = self.attention_maps[-1]  # [bsz, num_ref, h, w]
        if (
            not self.use_psuedo_attention_mask
            or len(self.attention_maps) < self.start_step
        ):
            return None, None
        text_attention_maps = torch.stack(self.attention_maps).mean(
            dim=0
        )  # [bsz, num_ref, h, w]
        ...
        psuedo_dummy_attention_mask = torch.ones(
            (bsz, seq_len_q), dtype=dtype, device=device
        )
        for i in range(num_ref):
            psuedo_box_mask = psuedo_attention_mask[..., i]
            psuedo_dummy_attention_mask = torch.clamp(
                psuedo_dummy_attention_mask - psuedo_box_mask, min=0
            )

        # post mask
        post_psuedo_dummy_attention_mask = psuedo_dummy_attention_mask.to(torch.bool)
        post_psuedo_dummy_attention_mask = (
            post_psuedo_dummy_attention_mask.repeat_interleave(head_size, dim=0)
        )

        psuedo_attention_mask = psuedo_attention_mask.repeat_interleave(
            self.num_tokens, dim=-1
        )
        psuedo_attention_mask = (1 - psuedo_attention_mask) * -10000.0  # mask to bias
        psuedo_dummy_attention_mask = psuedo_dummy_attention_mask.unsqueeze(
            -1
        ).repeat_interleave(self.num_dummy_tokens, dim=-1)
        psuedo_dummy_attention_mask = (1 - psuedo_dummy_attention_mask) * -10000.0
        psuedo_attention_mask = torch.cat(
            [psuedo_dummy_attention_mask, psuedo_attention_mask], dim=-1
        )
    ...
    def __call__(
        ...
        custom_attention_masks = self.prepare_attention_mask_qk(
            boxes,
            phrase_idxes,
        ...
        attention_mask_qk_image, attention_mask_qk_text, dummy_attention_mask = (
            custom_attention_masks
        )
        if attention_mask_qk_image is not None:
            attention_mask_qk_image = attention_mask_qk_image.view(
                batch_size, attn.heads, -1, attention_mask_qk_image.shape[-1]
            )
        if attention_mask_qk_text is not None:
            attention_mask_qk_text = attention_mask_qk_text.view(
                batch_size, attn.heads, -1, attention_mask_qk_text.shape[-1]
            )
        ...
        if not self.need_text_attention_map:
            ...
            self.get_text_attention_maps(
                attention_probs, boxes, phrase_idxes, attn.heads
            )
            hidden_states = torch.bmm(attention_probs, value)
            hidden_states = attn.batch_to_head_dim(hidden_states)

        # get psuedo attention mask for image: better start after some timesteps
        psuedo_attention_mask, psuedo_dummy_attention_mask = (
            self.get_psuedo_attention_mask(attn.heads)
        )
        if psuedo_attention_mask is not None:
            psuedo_attention_mask = psuedo_attention_mask.view(
                batch_size, attn.heads, -1, psuedo_attention_mask.shape[-1]
            )
        ...
        rf_attention_mask = (
            attention_mask_qk_image
            if attention_mask_qk_image is not None
            else rf_attention_mask
        )
        rf_attention_mask = (
            psuedo_attention_mask
            if psuedo_attention_mask is not None
            else rf_attention_mask
        )
        dummy_attention_mask = (
            psuedo_dummy_attention_mask
            if psuedo_dummy_attention_mask is not None
            else dummy_attention_mask
        )
        if not self.need_image_attention_map:
            ...
            if rf_attention_mask is not None:
                rf_attention_mask = rf_attention_mask.view(
                    batch_size * attn.heads, -1, rf_attention_mask.shape[-1]
                )
            ip_attention_probs = attn.get_attention_scores(
                new_query, ip_key, rf_attention_mask
            )
            # mask attention_probs in background
            ip_attention_probs = torch.where(
                dummy_attention_mask.unsqueeze(-1),
                torch.zeros_like(ip_attention_probs),
                ip_attention_probs,
            )
            if self.subject_scales is not None:
                # apply different scales to different subjects
                subject_scales = torch.tensor(
                    self.subject_scales,
                    dtype=ip_attention_probs.dtype,
                    device=ip_attention_probs.device,
                )
                subject_scales = (
                    subject_scales.unsqueeze(0)
                    .unsqueeze(0)
                    .repeat_interleave(self.num_tokens, dim=-1)
                )
                dummy_subject_scales = torch.ones(
                    (1, 1, 1),
                    dtype=ip_attention_probs.dtype,
                    device=ip_attention_probs.device,
                ).repeat_interleave(self.num_dummy_tokens, dim=-1)
                subject_scales = torch.cat(
                    [dummy_subject_scales, subject_scales], dim=-1
                )
